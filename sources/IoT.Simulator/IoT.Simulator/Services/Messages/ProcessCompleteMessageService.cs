...
namespace IoT.Simulator.Services
{
    ...
    public class ProcessCompleteMessageService : IProcessCompleteMessageService
    {
        private ILogger _logger;
        private string fileTemplatePath = @"./Messages/processcomplete.json";

        public ProcessCompleteMessageService(ILoggerFactory loggerFactory)
        {
        ...
        public async Task<string> GetMessageAsync(string deviceId, string moduleId)
        {
            ...
            string logPrefix = "ProcessCompleteMessageService".BuildLogPrefix();
            string messageString = await GetMessageAsync();

            if (string.IsNullOrEmpty(messageString))
                throw new ArgumentNullException(nameof(messageString), "DATA: The message to send is empty or not found.");

            _logger.LogTrace($"{logPrefix}::{artifactId}::processcomplete.json file loaded.");
